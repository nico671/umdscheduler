...
export async function generateSchedules(): Promise<void> {
    ...
    prohibitedProfessors.subscribe(value => currentProhibitedProfessors = value)();
    ...
    try {
        ...
        console.log("Sending request to generate schedules:", JSON.stringify(requestData));
        ...
        try {
            ...
            error.set(`Failed to connect to the backend server. Make sure it's running at http://127.0.0.1:5000 and reload the page.`);
        }
    } catch (err) {
        console.error("Error generating schedules:", err);
        error.set(err instanceof Error ? `Error: ${err.message}` : "An error occurred while generating schedules");
    } finally {
                ...
                data[0].sections.forEach((section: any) => {
                    if (section.instructors) {
                        section.instructors.forEach((instructor: string) => {
                            if (instructor && instructor !== "Instructor: TBA" && !professors.includes(instructor)) {
...
export function removeClasses(value: string): void {
    ...
    if (index !== -1) {
        ...
        currentProhibitedProfs = currentProhibitedProfs.filter((prof) => profsToKeep.has(prof));
        prohibitedProfessors.set(currentProhibitedProfs);

        // Update professor modals
        showProfessorModals.set(new Array(currentProhibitedProfs.length).fill(false));
...
export function closeViewProfessorModal(index: number, event?: CustomEvent): void {
    let profInfos: ProfessorViewInfo[] = [];
    viewProfessorInfo.subscribe(p => profInfos = p)();
...
export function sortSchedules(scheduleList: any[]): any[] {
    ...
    if (option === "rating") {
        return [...scheduleList].sort((a, b) => b["prof_weight"] - a["prof_weight"]);
    } else {
