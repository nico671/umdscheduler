...
def get_departments(term):
    ...
    response = BeautifulSoup(requests.get(url=url, headers=header).content, "html.parser")
...
def get_courses(departmentID, term):
    ...
    for e in response.find_all('div', class_='course'):
        ...
        grading_method = e.find('span', class_="grading-method").text.replace('\n','').replace('\t','')
...
def get_sections(course_code, term, restrictions):
    ...
    for s in response.find_all('div', class_='section'):
        ...
        section_id = s.find('span', class_='section-id').text.replace('\n','').replace('\t','')
...
def get_course(course_code, term):
    ...
    grading_method = response.find('span', class_="grading-method").text.replace('\n','').replace('\t','')
